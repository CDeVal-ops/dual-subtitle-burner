...
class SubMerger:
    ...
    def _ensure_middle_position(text: str) -> str:
        ...
        def adjust_y(y: float) -> float:
            # Push down from top region into upper-middle band, clamp to [560, 720]
            if y < 540:
    ...
    def _sanitize_and_map_bottom(
        ...
        for line in events2:
            ...
            if st.casefold() in passthrough_styles:
                # Preserve authored positions for signs/screen, unless only top-aligned (no absolute pos)
                if has_coords:
                    final_events.append(line)
                else:
                    if SubMerger._is_top_region(text):
                        # Only aligned to top (no coordinates): place in lower-middle band to avoid CN baseline
            ...
            if has_pos:
                if has_coords and SubMerger._is_top_region(text):
                    # Force top-like explicit positions into lower-middle band; set layer to stack with CN subs
                    p[9] = SubMerger._ensure_middle_position(text)
                    ...
                    # Respect authored explicit positioning when not top-like
                    final_events.append(line)
                if st in styles2:
                    kept_styles.add(st)
                continue
